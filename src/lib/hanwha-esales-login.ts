...
async function clickMainSearchButtonAndWaitForRows(
    ...
    if (expectedOrderNo) {
        throw new Error(`한화 e-Sales 주문 진행 조회에서 주문번호 ${expectedOrderNo} 행을 찾지 못했습니다. 조회 기간(${orderDateFromYmd}~${orderDateToYmd})을 확인해주세요.`);
    }
...
async function findOrderStatusRows(page: Page, shipToName: string) {
    const target = normalizeOrderMatchText(shipToName);
    if (!target) throw new Error('한화 e-Sales 주문상태 확인에 필요한 도착지명이 없습니다.');
    ...
    if (candidates.length === 0) {
        throw new Error(`한화 e-Sales 주문 진행 조회에서 도착지 "${shipToName}" 행을 찾지 못했습니다. 조회 기간과 도착지명을 확인해주세요.`);
    }
...
async function findMatchingOrderStatusRow(page: Page, input: {
    ...
    const checked = checkedSummaries.length ? ` 확인한 후보: ${checkedSummaries.join(' | ')}` : '';
    throw new Error(`한화 e-Sales 주문 진행 조회에서 도착지 "${input.shipToName}"의 주문은 찾았지만, 품목/수량/도착일이 현재 오더와 일치하는 행을 찾지 못했습니다.${checked}`);
}
...
export async function checkHanwhaESalesOrderStatus(input: {
    ...
    try {
        ...
        return {
            ...
            message: `한화 e-Sales 주문상태를 확인했습니다. 현재 상태: ${matched.status}`,
        };
    } catch (error) {
        return {
            ok: false,
